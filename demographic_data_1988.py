...
class DemographicData1988(Dataset):
    ...
    def bucketize_data(self, df):
        ...
        for i, age_range in enumerate(age_range_list):
            for code in code_list:
                if i == 0:
                    group = df.loc[(df['Age_1988'] < age_range) & (df['Code'] == code),
                            ['Both_Sexes_1988', 'Male_1988', 'Female_1988']].sum()
                    ...
                    group = df.loc[(df['Age_1988'] <= age_range[1]) & 
                            (df['Age_1988'] >= age_range[0]) & (df['Code'] == code),
                            ['Both_Sexes_1988', 'Male_1988', 'Female_1988']].sum()
                    group_list.append({code:{age_group_num_list[i]:group}})
        ...
        for i, group in enumerate(final_group_list):
            ...
            group.insert(1, 'Age_Group_1988', age_group_list[i], True)
            df_final = pd.concat([df_final, group], axis=0, ignore_index=True)
        # reorganize df and assign to class
        df_final = df_final.sort_values(['Code', 'Age_Group_1988'], ascending=[True, True])
        df_final.to_csv('demographic_buckets_1988.csv', index=False)
        self.df = df_final