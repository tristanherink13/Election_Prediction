...
class DemographicData2016(Dataset):
    # age/race/sex 2016 per state (200k rows to 195k x 7)

    def bucketize_data(self, df):
        # initialize variables
        df = self.df
        df = df[df.CODE != 0]
        df = df[df.AGE_2016 != 999]
                ...
                for sex in sex_codes:
                    for origin in origin_codes:
                        for race in race_codes:
                            if i == 0:
                                group = df.loc[(df['AGE_2016'] < age_range) & (df['CODE'] == code)
                                        & (df['SEX_2016'] == sex) & (df['ORIGIN_2016'] == origin)
                                        & (df['RACE_2016'] == race), ['POPESTIMATE2016']].sum()
                                ...
                                group = df.loc[(df['AGE_2016'] <= age_range[1])
                                        & (df['AGE_2016'] >= age_range[0]) & (df['CODE'] == code)
                                        & (df['SEX_2016'] == sex) & (df['ORIGIN_2016'] == origin)
                                        & (df['RACE_2016'] == race), ['POPESTIMATE2016']].sum()
                                group_list.append({code:{age_group_num_list[i]:group}})
        ...
        for i, group in enumerate(final_group_list):
            ...
            group.insert(1, 'Age_Group_2016', age_group_list[i], True)
            df_final = pd.concat([df_final, group], axis=0, ignore_index=True)
        # reorganize df, merge in other features, and assign to class
        df_final = df_final.sort_values(['Code', 'Age_Group_2016'], ascending=[True, True])
        df_final.to_csv('demographic_buckets_2016.csv', index=False)
        self.df = df_final