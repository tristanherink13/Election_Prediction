...
class DemographicData2008(Dataset):
    ...
    def bucketize_data(self, df):
        ...
        for i, age_range in enumerate(age_range_list):
            for code in code_list:
                for sex in sex_codes:
                    if i == 0:
                        group = df.loc[(df['AGE_2008'] < age_range) & (df['CODE'] == code)
                                & (df['SEX_2008'] == sex), ['POPESTIMATE2008']].sum()
                        ...
                        group = df.loc[(df['AGE_2008'] <= age_range[1]) & 
                                (df['AGE_2008'] >= age_range[0]) & (df['CODE'] == code)
                                & (df['SEX_2008'] == sex), ['POPESTIMATE2008']].sum()
                        group_list.append({code:{age_group_num_list[i]:group}})
        ...
        for i, group in enumerate(final_group_list):
            ...
            group.insert(1, 'Age_Group_2008', age_group_list[i], True)
            df_final = pd.concat([df_final, group], axis=0, ignore_index=True)
        # reorganize df, merge in other features, and assign to class
        df_final = df_final.sort_values(['Code', 'Age_Group_2008'], ascending=[True, True])
        df_final['SEX_2008'] = sex_codes*969
        df_final.to_csv('demographic_buckets_2008.csv', index=False)
        self.df = df_final